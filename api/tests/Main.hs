...
DEPNS_PKGIDS=`for pkg in base test-framework-hunit test-framework-quickcheck2 hslogger ; do echo -n -package-id= ; ghc-pkg --global --user --package-db=$EXTRA_PKGDB --simple-output field $pkg id | head -n1 ; done`
...
import qualified SequenceopsCase
import qualified SequenceopsProp
...
tests = map (uncurry testGroup) 
    ...
    , ("SequenceopsCase", SequenceopsCase.tcases)
    , ("SequenceopsProp", SequenceopsProp.tprops)
    ]
